...
Lorsqu'on parallélise la fonction `mise_a_jour` de parametres.cpp, il peut y avoir un data race. En effet, 2 threads peuvent écrire en même temps dans `galaxie_next`, lorsque l'un va coloniser les planètes autours et l'autre va changer l'état de la planère sur laquelle il travaille. En pratique, ce data race n'arrive pas car ces deux évènements n'arrivent jamais en même temps. En effet, pour que cela arrive il faut que les deux threads travaillent en même temps sur 2 planètes adjacentes. Cela revient à ce qu'il y ait autant de thread que de lignes. Ce qui n'est pas le cas en pratique. 
