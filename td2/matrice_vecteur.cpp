...
int main( int nargs, char* argv[] )
{
    ...
    std::vector<double> resultat_tot(N);
    for (int i = rank *Nloc; i< (rank +1)*Nloc; i++ ) {
        for (int j = 0; j < N ; j ++){
            resultat[j]=A(j,i)*u[i] ;
        }
        MPI_Reduce(resultat.data(), resultat_tot.data(), N, MPI_DOUBLE, MPI_SUM,0,MPI_COMM_WORLD);
    }
    std::cout << "Le résultat est : " << resultat<< std::endl ;
    std::cout << "Le résultat est : " << resultat_tot << std::endl ;
	MPI_Finalize();
	return EXIT_SUCCESS;
}